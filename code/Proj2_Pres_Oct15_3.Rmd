...
From both formulas, we determined abnormal values with index 182 and 42. Abnormal height of index 42 was changed, but the formula did not provide a reasonable alternative for abnormal body fat percentage of index 182. So, index 182 should be deleted.
...
# Index 216 has bodyfat 45.1, much larger than other entries
df[df$BODYFAT==max(df$BODYFAT),]

# Index 39 has weight 363.15, much larger than other entries
df[df$WEIGHT==max(df$WEIGHT),]
```

Index 172 did not show up when detecting anomaly with body fat/density formula, suggesting that the density aligns with the abnormal body fat percentage. Therefore, it should be deleted; Index 216 and index 39 both have parameters higher than any other entries, and we considered these entries not helpful to a predictive model. They should be deleted.
...
So, we need to pick 10,10,10,9,9,8,9,9,9,8,6 people from respective age groups. This smaller data set will simulate 2022 age distribution.
...
For our linear regression model, we first checked one simulated data set with size 97, and then checked 5 sets combined with size 485.
...
# prediction range is 17.6, meaning +/- 8.8
prediction_range
...
# prediction range is 16.3, meaning +/- 8.15
prediction_range
...
These models' R-squared values performed better than with the smaller data set, the residual plot did not have a pattern, and the Bodyfat Percentage~Abdomen Circumference model produced a smaller prediction range 16.3. However, the QQ-plot displayed a shift on both ends, undermining the model's robustness.
...
### remove Index 39, 172, 182, 216
body.fat <- body.fat.ori[c(1:38, 40:171, 173:181, 183:215, 217:dim(body.fat.ori)[1]),]
```
...
```{r}
### for each age, count how many pieces of data does this age have 
sample.cnt <- body.fat |>
  group_by(AGE) |>
  summarise(count = n())

used.age <- ages[c(5:(length(ages)-3), length(ages)-1)]
### group sample age by official age intervals
j <- 1
i <- 1
sample.med <- c()
...
new.count ### calculate how many people at different age interval
```

```{r}
### build a dataframe consisting of sample age and official age and then calculate weights
...
```{r}
weights.use <- c()
### calculate vector of weight which will be used
j <- 1
i <- 1
while(i <= dim(sample.cnt)[1]){
  if ((sample.cnt$AGE[i] <= used.age[j]+2) & (sample.cnt$AGE[i] >= used.age[j]-2)){
...
# prediction range is 14.5, meaning +/- 7.25
prediction_range
